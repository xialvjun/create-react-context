...
const Login = AuthContextInstance.Hoc("auth")((p: { auth: AuthContext }) => (
  <div>{p.auth.state.logined ? <div>logout</div> : <div>login</div>}</div>
));

const NoActionContext_Person = new Context({ name: "xialvjun", age: 28 });
function SomeThing({
  person,
  extra
}: {
  ...
  return (
    <div onClick={_ => person.setState({ age: person.state.age + 1 })}>
      {person.state.name}
    </div>
  );
}
const WithPerson = NoActionContext_Person.Hoc("person")(SomeThing);
// only extra is needed in typescript
<WithPerson extra="asdfdsf" />;
...
function OldRenderProps({ render }) {
    ...
    <div>
      ...
      <Compose
        mapper={{
          counter: Counter,
          auth: <Auth>{_ => _}</Auth>,
          old_render_props: ({ children }) => (
            <OldRenderProps render={children} />
          )
        }}
      >
